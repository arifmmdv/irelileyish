<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {!! Form::open(['route' => ['languages.update',$language->id], 'method' => 'POST']) !!}
                    <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <div class="flex flex-wrap">
                                <div class="w-1/2 p-2">
                                    <label for="title" class="block font-medium text-gray-700">Title</label>
                                    <input type="text" name="title" id="title" value="{{$language->title}}" autocomplete="given-name" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                </div>

                                <div class="w-1/2 p-2">
                                    <label for="locale" class="block font-medium text-gray-700">Locale</label>
                                    <input type="text" name="locale" id="locale" value="{{$language->locale}}" autocomplete="given-name" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                                ...
                                <div class="w-1/2 p-2">
                                    ...
                                    <select id="dir" name="dir" autocomplete="dir" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                        <option value="ltr" @if($language->dir == 'ltr')selected @endif>Left to Right</option>
                                        <option value="rtl" @if($language->dir == 'rtl')selected @endif>Right to Left</option>
                                    </select>
                                </div>
                            </div>
                        </div>
